...
`@script`
By simply looking at a time series is hard to choose which of our models we should fit. These two time series look very similar, but the model we choose is very important. If we make the wrong choice of model to fit we can end up with poor forecasts.
...
We talked a lot about autocorrelation when we introduced the AR model.
 
*The ACF tells us how much our time series is correlated with itself at a lag of n

*The PACF tells us how much our time series is correlated with itself at a lag of n after removing the correlations caused by smaller lags
...
`@script`
If the data is from AR model then the ACF will tail off gradually, whilst the PACF will drop off suddenly after lag p. In this example, where we have an AR(1) model, you can see that the PACF drops off after lag 1. The area shaded in blue shows the values of ACF and PCAF which are not statistically significant

*If the data is characterised by an MA model then the ACF will drop off suddenly after lag q and the PACF will tail off gradually. Here we have an MA(1) model, you can see that the ACF drops off after lag 1. Here the amplitude of PACF that tails off gradually and it oscillates between positive and negative.

We can use these plots to pick an appropriate model, and parameters p and q
...
`@script`
If the data is characterised by an ARMA model then both the ACF and the PACF tail off. In this case it is harder to pick the model parameters p and q. We will come back to this challenge in a later chapter.
...
*The ACF and PACF trail off so we'll use an ARMA model
...
*To fit a model, first we instantiate the model object, passing it the data and the order. Lets use p and q both equal to 1. This is the simplest ARMA model.
...
`@script`
Using the fitted model we can now make predictions about future flu rate using the get_predictions method. We'll discuss forecasts further in a later video but lets zoom in for a closer look at our predictions
