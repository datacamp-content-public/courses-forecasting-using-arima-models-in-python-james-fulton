...
`@script`
If the data is from AR model then the ACF should tail off gradually. The PACF should drop off suddenly after lag p.

In this example, you can see that the PACF drops off after lag 1. This suggests an AR(1) model. 

Note that the area shaded in blue shows the values of ACF and PCAF which are not statistically significant
...
In this plot, you can see that the ACF drops off after lag 1 and  amplitude of PACF tails off gradually but oscillates between positive and negative. This suggests an MA(1) model
...
`@script`
If the data is characterised by an ARMA model then both the ACF and the PACF tail off. In this case it is harder to pick the model parameters p and q. We will come back to this challenge later.
...
`@script`
We import the ARMA model as before and instantiate the model.
...
`@script`
Using the fitted model we can now make predictions about future flu rate using the get_predictions method. We'll discuss forecasts further in a later video but lets zoom in for a closer look to see how our model performs
